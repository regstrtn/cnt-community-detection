	...
	for c in commu:
		...
		if len(intra_inter[c])>1:
			...
			for l in layer:
				...
				I_layer=float(I_layer)/2.0
				#sm_layer=float(m_layer)/2.0
				m_layer1=float(m_layer1)/2.0
				if I_layer>-1 and m_layer>0: # and m_layer1>0:
					mod=((I_layer/m_layer)-((d_layer/(2*m_layer))*(d_layer/(2*m_layer))))
				else:
					mod=0	
				#if n_layer > 0:
...
#fp1=open('./mQ_modified_new_march1/single_layer_complete_clique/p40.txt','w')
config = ["new_config_multi","new_config_single"]
commu_type =["multi_layer","single_layer"]

str0 = "./new_config_mod/mQ_modified_new_march1/"
