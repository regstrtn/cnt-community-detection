import math
import os
#For intra layer communitiez consider nodes' overall degrees in null model, for cross layer communities keep it same as MultiMod
...
#p_list=[40]
#for p in p_list:
#ss='./march17/raphael_network_l2_100_100.txt'
...
str0 = "./new_config_mod/mQ_modified_new_march18/"
...
for p in p_list:
	#ss='./march16/single/march1_single_single_0_1_'+str(p)+'.txt'	
	#ss='./march16/mQ_modified_march16_100_5_config1_no_ext.txt'
	ss='./march17/raphael/raphael_network_mu_'+str(p)+'.txt'
	fp1=open(ss,'w')
	for perturb in range(1,101):
		if perturb%10 !=0 or perturb==90:
			continue
		frac=float(perturb)/100.0
		str1='./generation_network_raphael/density/config1_f'+str(frac)+'_mu'+str(p)+'.txt'
		#str1='./config1_no_ext/config1_'+str(frac)+'.txt'
		#str1='./new_single_layer/new_config_single/with_0.1_coupling/Test_config('+str(p)+'%)/test_config'+str(frac)+'.txt'
...
#p_list=[1]
for p in p_list:
	ss='./march17/single/mQ_single_single_1_'+str(p)+'.txt'	
...
#p_list=[1]
for p in p_list:
	ss='./march17/single/mQ_single_multi_1_'+str(p)+'.txt'	
	#ss='./mQ_modified_new_march1_corrected/mQ_modified_march1_100_5_config5_1_no_ext.txt'
	fp1=open(ss,'w')
	for perturb in range(0,101):
		frac=float(perturb)/100.0
