...
for fr in p:
	...
	for perturb in range(0,101):
		...
		str1='./config_complete_clique/Test_config('+str(fr)+'%)/test_config'+str(frac)+'.txt'
		if not os.path.exists('./config_complete_clique/Test_config('+str(fr)+'%)'):
			    os.makedirs('./config_complete_clique/Test_config('+str(fr)+'%)')
		fp=open(str1,'w')
