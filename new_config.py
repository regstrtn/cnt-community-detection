...
for ct in coupling_type:
	for fraction_filename in range(10, 101, 10):
		for perturb in range(0,101):
			...
			str1='./new_config_multi/'+ct+'/Test_config('+ str(fraction_filename) +'%)/test_config'+str(frac)+'.txt'
			if not os.path.exists('./new_config_multi/'+ct+'/Test_config('+ str(fraction_filename) +'%)'):
			    os.makedirs('./new_config_multi/'+ct+'/Test_config('+ str(fraction_filename) +'%)')
